...
const getCoder = function(encoding) {
	...
	if (Object.keys(coder.charListEnc).length === 0) {
		...
		let extCharsEnc = coder.extCharsEnc ?? coder.extChars;
		let escCharsEnc = coder.escCharsEnc ?? coder.escChars;
		for (let i = 0; i < extCharsEnc.length; i++) {
			coder.extCharListEnc[extCharsEnc[i]] = escCharsEnc[i];
		}

		let extCharsDec = coder.extCharsDec ?? coder.extChars;
		let escCharsDec = coder.escCharsDec ?? coder.escChars;
...
const decode = function (string, encoding) {
	...
	for (const element of string) {
		result += coder.charListDec[element] ?? ' ';
	}
...
const time = {
	...
	decode: function(value) {
		...
		const result = new Date(value.replace(
			/^(..)(..)(..)(..)(..)(..)(.)?.*$/,
			century + '$1-$2-$3 $4:$5:$6:$700 UTC'
		));
		const match: any = RegExp(/(..)([-+])$/).exec(value);
		if (match && match[1] != '00') {
			let diff = match[1] * 15;
			if (match[2] == '+') {
				diff = -diff;
...
const message = {
	encode: function(value) {
		...
		if (typeof message === 'string' && message) {
			...
			if (!encoded) {
				let encoding: any = encodings.LATIN1;
	...
	decode: function(value, skipUdh) {
		...
		let encoding: any = this.data_coding & 0x0F;
		if (!encoding) {
...
const billing_identification = {
	encode: function(value) {
		...
		let result = Buffer.alloc(value.data.length + 1);
...
const callback_num = {
	encode: function(value) {
		...
		let result = Buffer.alloc(value.number.length + 3);
...
const callback_num_atag = {
	encode: function(value) {
		...
		let result = Buffer.alloc(value.display.length + 1);
...
tlvs = {
	...tlvs,
	alert_on_msg_delivery : { ...tlvs.alert_on_message_delivery },
	failed_broadcast_area_identifier : { ...tlvs.broadcast_area_identifier },
...
export class Defs {
	public static readonly encodings = encodings;
	public static readonly filters = filters;
	public static readonly gsmCoder = gsmCoder;
	public static readonly consts = consts;
	public static readonly commands = commands;
	public static readonly commandsById = commandsById;
	public static readonly types = types;
	public static readonly tlvs = tlvs;
	public static readonly tlvsById = tlvsById;
